...
├── CONTRIBUTING.md            ← コントリビューション条件（軽量CLA）
├── package.json
├── tsconfig.json              ← "types": ["node"] が必須
├── flyde-mc.config.json        ← ログレベル設定（git 管理対象）
├── _nodes/
│   ├── index.flyde.ts         ← 全ノード兼 Flyde エディタ用エントリ
...
└── flows/                     ← ユーザーが .flyde ファイルを作る場所（*.flyde は .gitignore 対象）
```
...
npm run lang -- ja_JP    # 言語を切り替え（_i18n/ にある任意のロケールを指定可能）
```

言語切替は `scripts/set-lang.js` が `index.flyde.ts`・`_catalog.ts` の2ファイルを書き換える。
...
テストフローは `tests/` に格納。ファイル名はセクション番号ベース（`test-06-gameplay.flyde` 等）。

テストフローは `../_nodes/index.flyde.ts` を直接参照するため、ビルドは不要（`npm install` のみで実行可能）。

テスト環境（別フォルダ）で `npm link` を使って確認する。  
詳細手順は `tests/test-spec.md` の「テスト実行手順」を参照。
...
Flyde エディタはワークスペース内の `*.flyde.ts` / `*.flyde.js` を自動スキャンしてメニューに登録する。`_nodes/index.flyde.ts`（TSソースそのまま）がそのまま検出されるため、ビルドは不要（独自のビルドスクリプトは持たない）。

### Flyde モジュール分離とプロセスブリッジ

Flyde はフロー再起動時にモジュールキャッシュをクリアして再 `require()` する。そのたびに `context-manager.ts`/`ws-server.ts` 等のモジュールスコープの状態（シングルトン）が初期化され直してしまう。
...
この仕組みにより、フロー再起動でモジュールが再読み込みされても接続状態が引き継がれる。
...
import i18n from './_i18n/ja_JP.json'  // ← npm run lang -- en_US で en_US.json に切り替わる
...
言語切替は `npm run lang -- ja_JP`（または `node scripts/set-lang.js ja_JP`）で行う。  
`scripts/set-lang.js` が以下の2ファイルを同時に書き換える：
...
GitHub リポジトリ自体が配布チャネル。ユーザーは `git clone`（または GitHub の「Download ZIP」）して `npm install` するだけで、`_nodes/index.flyde.ts` がそのままFlydeのノードメニューに表示される（ビルド不要）。独自のzip生成・リリース配布スクリプトは持たない。

### ライセンス構成

有料版（フル版）は終了し、ソースコードを含めて単一ライセンスで公開している。

| ライセンス | ライセンスファイル |
|---|---|
| PolyForm Noncommercial License 1.0.0 | `LICENSE.md` |

- 非商用目的（個人・学校・公的機関等）は無期限に無料。商用利用は Mming Lab への連絡が必要
- コントリビューション（PR）は `CONTRIBUTING.md` の条件に同意したものとみなす
...
`_maps/*.json` はリポジトリに含まれており、`git clone` でそのまま手に入る。
